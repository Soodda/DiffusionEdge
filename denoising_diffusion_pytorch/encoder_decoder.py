        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks+1):
                h = self.up[i_level].block[i_block](
                    torch.cat([h, hs.pop()], dim=1), temb)#每个残差块的输入是 当前特征图 和 下采样路径中对应分辨率的特征图 的拼接
...
class Decoder(nn.Module):
    #mid + up 无temb 潜在空间向量z--->h(h,w)像素值压缩到【-1,1】
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels,
...
class LatentRescaler(nn.Module):
    ...
    def __init__(self, factor, in_channels, mid_channels, out_channels, depth=2):
        ...
        self.factor = factor #如果 factor > 1，则进行上采样；如果 factor < 1，则进行下采样
...
class FirstStagePostProcessor(nn.Module):
    #提取 pretrained_model 训练好的 latent 表示，并进行后续处理
...
class DiagonalGaussianDistribution(object):
    ...
    def __init__(self, parameters, deterministic=False):
        ...
        self.mean, self.logvar = torch.chunk(parameters, 2, dim=1) #均值
        self.logvar = torch.clamp(self.logvar, -30.0, 20.0) #方差对数
    ...
    def sample(self):
        #x = mean + std * eps，其中 eps 是从标准正态分布中采样的噪声
...
class AutoencoderKL(nn.Module):
    def __init__(self,
        ...
        self.encoder = Encoder(**ddconfig) #x---》z
        self.decoder = Decoder(**ddconfig) #z---》x‘
        self.down_ratio = 2 ** (len(ddconfig['ch_mult']) - 1)
        self.loss = LPIPSWithDiscriminator(**lossconfig)
        assert ddconfig["double_z"]
        self.quant_conv = torch.nn.Conv2d(2*ddconfig["z_channels"], 2*embed_dim, 1)#将编码器的输出映射到潜在空间的均值和方差
        self.post_quant_conv = torch.nn.Conv2d(embed_dim, ddconfig["z_channels"], 1) #将潜在表示映射回解码器的输入维度
