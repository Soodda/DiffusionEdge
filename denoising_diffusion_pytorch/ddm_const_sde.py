...
class DDPM(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        beta_schedule = 'cosine',#噪声调度方式
        p2_loss_weight_gamma = 0., # p2 loss weight, from https://arxiv.org/abs/2204.00227 - 0 is equivalent to weight of 1 across time - 1. is recommended
        p2_loss_weight_k = 1, #调整损失权重的参数
        original_elbo_weight=0.,
        ddim_sampling_eta = 1.,#采样时的噪声系数
        clip_x_start=True,
        train_sample=-1,
        input_keys=['image'],
        start_dist='normal',
        ...
        **kwargs
        ...
        if beta_schedule == 'linear':
            betas = linear_beta_schedule(timesteps)#噪声调度参数
        elif beta_schedule == 'cosine':
            betas = cosine_beta_schedule(timesteps, s=1e-4)
        else:
            raise ValueError(f'unknown beta schedule {beta_schedule}')
        # betas[0] = 2e-3 * betas[0]
        alphas = 1. - betas #每个时间步的噪声系数
        alphas_cumprod = torch.cumprod(alphas, dim=0) #累计噪声系数
        alphas_cumprod_prev = F.pad(alphas_cumprod[:-1], (1, 0), value = 1.) #前一个时间步的累积噪声系数
        ...
        # helper function to register buffer from float64 to float32注册缓冲区
        ...
        # calculations for diffusion q(x_t | x_{t-1}) and others  扩散
        ...
        # calculations for posterior q(x_{t-1} | x_t, x_0) 逆过程
...
class LatentDiffusion(DDPM):
    ...
    def pred_xtms_from_xt(self, xt, noise, C, t, s):
        # 去噪 得到去噪后前一个时间步的潜在空间表示
        # noise = noise / noise.std(dim=[1, 2, 3]).reshape(C.shape[0], *((1,) * (len(C.shape) - 1)))
        time = t.reshape(C.shape[0], *((1,) * (len(C.shape) - 1))) #当前时间步
        s = s.reshape(C.shape[0], *((1,) * (len(C.shape) - 1))) #时间步间隔
        mean = xt - C * s - s / torch.sqrt(time) * noise #计算前一个时间步潜在表示的均值
    ...
    def p_losses(self, x_start, t, *args, **kwargs):
        ...
        # K = -1. * torch.ones_like(x_start)
        # C = noise - x_start  # t = 1000 / 1000
        C = -1 * x_start  # U(t) = Ct, U(1) = -x0 噪声函数
        # C = -2 * x_start               # U(t) = 1/2 * C * t**2, U(1) = 1/2 * C = -x0
        x_noisy = self.q_sample(x_start=x_start, noise=noise, t=t, C=C)  # (b, 2, c, h, w)
    ...
    def sample(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True):
        ...
        if self.scale_by_std:
            ...
            z = z.detach()
        #print(z.shape)
        x_rec = self.first_stage_model.decode(z) #从潜在空间还原
