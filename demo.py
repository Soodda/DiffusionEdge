...
def main(args):
    ...
    if data_cfg['name'] == 'edge':
        ...
        #print(f"Dataset created with {len(dataset)} samples.")  # 打印数据集大小
        # 检查输入目录的内容
        import os
        print("Input directory contents:", os.listdir(args.input_dir))
    else:
        raise NotImplementedError
    ...
    # 检查数据加载器的大小
    #print(f"DataLoader created with {len(dl.dataset)} samples and batch size of {cfg.sampler.batch_size}.")
